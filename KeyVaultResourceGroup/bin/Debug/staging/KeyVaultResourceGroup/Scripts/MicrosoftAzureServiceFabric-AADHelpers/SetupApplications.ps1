...
. $PSScriptRoot\Common.ps1 -userName $userName -password $password
...
$ASFWebAppId = $ConfigObj.WebAppId
Write-Host ("##vso[task.setvariable variable=ASFWebAppId;]$ASFWebAppId")
Write-Host $env:ASFWebAppId
Write-Host ("  `"clusterApplication`":`"{0}`"," -f $ConfigObj.WebAppId)

$ASFClientAppId = $ConfigObj.NativeClientAppId
Write-Host ("##vso[task.setvariable variable=ASFClientAppId;]$ASFClientAppId")
Write-Host $env:ASFClientAppId
