...
if (-not $UseExistingClusterConnection)
{
   ...
   $conn = Connect-ServiceFabricCluster -ConnectionEndpoint $clusterEndpoint ` -KeepAliveIntervalInSec 10 ` -X509Credential -ServerCertThumbprint $clusterThumbprint `-FindType FindByThumbprint -FindValue $clusterThumbprint ` -StoreLocation CurrentUser -StoreName My

 Write-Host $conn
 Write-Host "cluster connection : " $conn.ConnectionEndpoint
}
